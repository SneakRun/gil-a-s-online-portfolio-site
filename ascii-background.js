...
// Move these to the top of the file, outside any function
const asciiChars = "@$B%8&WM#*oahkbdpqwmZO0QLCJUYXzcvunxrjft/\\|()1{}[]?-_+~<>i!lI;:,\"^`'. ";
let zOffset = 0;
const timeFactor = 0.0015;
const noiseScale = 0.06;

// Keep the noise function and its helper functions as they are
...
function initHeaderAscii() {
    const headerCanvas = document.getElementById('header-ascii-canvas');
    if (!headerCanvas) return;

    const headerCtx = headerCanvas.getContext('2d');
    let headerFontSize, headerCols, headerRows;
    ...
    function drawHeaderAscii() {
        headerCtx.clearRect(0, 0, headerCanvas.width, headerCanvas.height);
        headerCtx.fillStyle = getComputedColor('--main-text-color');
        ...
        zOffset += timeFactor;
        requestAnimationFrame(drawHeaderAscii);
    }
